...
public class RejectLeaveRequestController extends BaseRequiredAuthenticationController {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp, User user) throws ServletException, IOException {
        ...
        if (!isAdmin && !isManager) {
            resp.sendError(HttpServletResponse.SC_FORBIDDEN, "Bạn không có quyền từ chối đơn xin nghỉ phép.");
            return;
        }
        ...
        if (leaveRequest.getStatusID() != 3) { // Chỉ cho phép từ chối đơn đang "Chờ duyệt"
            resp.sendError(HttpServletResponse.SC_BAD_REQUEST, "Đơn này đã được xử lý trước đó.");
            return;
        }

        // Cập nhật trạng thái đơn thành "Bị từ chối" (StatusID = 2)
        leaveRequest.setStatusID(2);
        leaveRequest.setApprovedBy(user.getUserID()); // Ghi lại người từ chối
